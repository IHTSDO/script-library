...
public class INFRA9606_RetermMeasurementFindings extends BatchFix {
	...
	enum Pattern { RAISED_X, Y_WITH_X, X_RAISED, ALL, CHECK_PT_ONLY, SKIP_AND_REPORT }
	
	Set<String> highSynonyms = new HashSet<>();
	{
		highSynonyms.add("high");
		highSynonyms.add("raised");
		highSynonyms.add("elevated");
		highSynonyms.add("increased");
		highSynonyms.add("above reference range");
	}

	Map<Pattern, String> toTemplateMap = new HashMap<>();
	{
		toTemplateMap.put(Pattern.ALL, "#x# above reference range");
	}
	...
	private void tweakCsIfRequired(Task t, Concept c) throws TermServerScriptException {
		for (Description d : c.getDescriptions(ActiveState.ACTIVE)) {
			if (StringUtils.initialLetterLowerCase(d.getTerm())) {
				d.setTerm(StringUtils.capitalizeFirstLetter(d.getTerm()));
			}
	...
	private PatternOfX identifyPattern(Description d, boolean isFSN) throws TermServerScriptException {
		...
		for (String synonym : highSynonyms) {
			if (termLower.startsWith(synonym)) {
				String x = term.substring(synonym.length() + 1);
				return new PatternOfX(Pattern.RAISED_X, x);
				...
				return new PatternOfX(Pattern.X_RAISED, x);
			}
		}
